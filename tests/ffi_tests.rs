...
fn test_ffi_encrypt_decrypt() {
    ...
    let decrypted_text = unsafe { std::slice::from_raw_parts(decrypted.data.data, decrypted.data.len) };
...
fn test_ffi_encrypt_decrypt_fields() {
    ...
    let decrypted = ffi::decrypt_fields(encrypted.data.data, encrypted.data.len, fields_cstring.as_ptr(), key.as_ptr());
    assert_eq!(decrypted.error_code, 0);
    assert_ne!(decrypted.data.len, 0);
    assert_ne!(decrypted.data.data, std::ptr::null_mut());

    let decrypted_str = unsafe { str::from_utf8_unchecked(std::slice::from_raw_parts(decrypted.data.data, decrypted.data.len)) };
...
fn test_ffi_encrypt_decrypt_fields_in_batch() {
    ...
    let decrypted = ffi::decrypt_fields_in_batch(encrypted.data.data, encrypted.data.len, fields_cstring.as_ptr(), key.as_ptr());
    assert_eq!(decrypted.error_code, 0);
    assert_ne!(decrypted.data.len, 0);
    assert_ne!(decrypted.data.data, std::ptr::null_mut());
